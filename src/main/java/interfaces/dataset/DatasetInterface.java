...
public interface DatasetInterface {
     ...
     * @return a JSON object with an error or warning message or a message indicating the correct operation
     ...
     * @param datasetName is the dataset name used inside the Learning Orchestra
     * @return a JSON object with an error or warning message or a message indicating the correct operation
     * @summary This method is responsible to update a dataset from a URI synchronously,
     ...
     */
    JsonObject updateDatasetSync(String datasetName);
     ...
     * is updated or not.
     */
    JsonObject updateDatasetAsync(String datasetName);
     ...
     * locks the caller
     * until the deletion finished. If a dataset was used by another task (Ex. projection, histogram, pca, tuning
     * and so forth),
     ...
     * does not lock the caller
     * until the deletion finished. Instead, it returns a ticked for a future use. The caller uses the ticket
     * (a URL or a message or many more).
     * If a dataset was used by another task (Ex. projection, histogram, pca, tuning and so forth),
     * it cannot be deleted.
     */
     ...
     * @summary This method is responsible for retrieving the dataset content
     */
     ...
     * @summary This method is responsible for retrieving the dataset content, precisely the first 10 tuples
     */
